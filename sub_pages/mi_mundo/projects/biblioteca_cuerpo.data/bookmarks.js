 ...
 */
 ...
 function addBookmark() {
     ...
     var entry = '<li id="newOne" title="' + tags + '" time="' + secs + '"><a target="_blank" href="' + link + '">' + name + '</a><p> ' + link + '</p></li>';
 ...
 function decorate(entry, tag) {
     ...
     /** Remove trailing ", ". */
     txt = txt.replace(/, $/, ".");
     entry.append(txt + "</li></ul><hr id=\"hrcolor\"> ");
 }
 ...
 function editBookmark (selector) {
     ...
     $('#saveBookmark').off('click').one('click', function(){
         ...
         selector.find('> p').text( $('#newLink').val());
 ...
 function setupEditRemove () {
     ...
     li.mouseenter(function(){
         ...
         var edit = $('<span class="context">&nbsp;✏️&nbsp;</span>').insertAfter(clia);
         edit.off('click').one('click', function(){
             editBookmark(cli);
         });
         remove.off('click').one('click', function(){
             if ( confirm( "Eliminar información?" ) ) {
                 cli.remove();
             }
         });
     });
 ...
 /**
  * Load our bookmarks from the URL `biblioteca_cuerpo.data`, and setup our
  * initial state + listeners.
  */
 function setup () {
 
     $("#bookmarks").load("biblioteca_cuerpo.data",function() {
 ...
 function saveDataFile() {
     ...
     var filename = "biblioteca_cuerpo.data";
